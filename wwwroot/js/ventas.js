...
(function setupClienteSelect() {
    ...
    function poblarSelectClientes(lista) {
        ...
        $selectCliente.add(new Option('— Lista de clientes —', ''));
        for (const cliente of lista) {
            if (cliente.id == null) continue;
            const opcion = new Option(`${cliente.nombre} — Dni: ${cliente.dni}`, cliente.id);
        ...
        try {
            ...
            if (!idCliente) {
                await Swal.fire({
                    ...
                    background: '#212529',
                    color: '#f8f9fa',
                    text: 'Se debe seleccionar un cliente de la lista antes de generar la factura.',
                    icon: 'error',
                    confirmButtonText: 'Ok'
                });
                return;
